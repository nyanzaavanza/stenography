...
class Stegno:
    ...
    def main(self, root):
        root.title('SECURITY USING CYPER CRYPTOGRAPHY')
        root.geometry('900x600')
        root.resizable(width=False, height=False)
        f = Frame(root)
        self.encodez = tkinter.PhotoImage(file=r"C:\Users\harve\PycharmProjects\pythonProject\pythonProjectdecodemyshit\venv\button\encode.png")
        self.decodex = tkinter.PhotoImage(file=r"C:\Users\harve\PycharmProjects\pythonProject\pythonProjectdecodemyshit\venv\button\decode.png")
        self.cool = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\pradip.png")
        self.cool1 = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\parul.png")
        self.sem = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\sem.png")
        title = Label(f,image=self.cool)
        title.config(font=('courier' ,33))
        title.grid(pady=10)


        b_encode = Button(f,image=self.encodez,command= lambda :self.frame1_encode(f), padx=14)

        b_decode = Button(f,image=self.decodex,padx=14,command=lambda :self.frame1_decode(f))

        ascii_art = Label(f, image = self.cool1)
        sem = Label(f, image=self.sem)
    ...
    def frame1_decode(self,f):
        f.destroy()
        d_f2 = Frame(root)
        self.hero = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\dselect.png")
        self.hero1 = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\dcancel.png")
        label_art = Label(d_f2, text='٩(^‿^)۶')
        label_art.config(font=('courier',90))
        label_art.grid(row =1,pady=50)
        l1 = Label(d_f2, text='Select Image To decode:')
        l1.config(font=('courier',18))
        l1.grid()
        bws_button = Button(d_f2, image = self.hero, command=lambda :self.frame2_decode(d_f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(d_f2, image = self.hero1, command=lambda : Stegno.home(self,d_f2))
    ...
    def frame1_encode(self,f):
        f.destroy()
        f2 = Frame(root)
        self.select = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\Select.png")
        self.cancel = tkinter.PhotoImage(file=r"C:\Users\harve\Downloads\cancel.png")
        label_art = Label(f2, text='▂▃▅▇█▓▒░۩۞۩        ۩۞۩░▒▓█▇▅▃▂\'')
        label_art.config(font=('courier',20))
        label_art.grid(row =1,pady=50)
        l1= Label(f2,text='Select the Cover Image in which \nyou want to hide message :')
        l1.config(font=('courier',18))
        l1.grid()

        bws_button = Button(f2,image=self.select,command=lambda : self.frame2_encode(f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(f2,image=self.cancel, command=lambda : Stegno.home(self,f2))
        back_button.config(font=('courier',18))
        back_button.grid(pady=15)
        back_button.grid()
        f2.grid()
